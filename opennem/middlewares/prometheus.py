...
class WebService(Site):
    ...
    def __init__(self, crawler):
        ...
        self.spr_item_scraped = Gauge("spr_items_scraped", "Spider items scraped", ["spider"])
        self.spr_item_dropped = Gauge("spr_items_dropped", "Spider items dropped", ["spider"])
        ...
        self.spr_downloader_response_count = Gauge("spr_downloader_response", "...", ["spider"])
        ...
        self.spr_memdebug_gc_garbage_count = Gauge("spr_memdebug_gc_garbage", "...", ["spider"])
        ...
        self.spr_offsite_filtered_count = Gauge("spr_offsite_filtered", "...", ["spider"])
    ...
    def request_stats(self):
        for i in ["GET", "PUT", "DELETE", "POST"]:
            ...
            if count > 0:
                self.spr_downloader_request_count.labels(spider=self.name, method=i).set(count)
    ...
    def response_stats(self):
        ...
        for i in ["200", "404", "500"]:
            ...
            self.spr_downloader_response_status_count.labels(spider=self.name, code=i).set(status)
    ...
    def logging_stats(self):
        for i in ["DEBUG", "ERROR", "INFO", "CRITICAL", "WARNING"]:
            ...
            self.spr_log_count.labels(spider=self.name, level=i).set(level)
