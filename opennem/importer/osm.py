...
def get_import_osm_data(file_name: str = "osm_ways.csv") -> list[OSMImportCSVSchema]:
    ...
    if not osm_ways_path.is_file():
        raise Exception(f"Could not import photo file data: {str(osm_ways_path)}")
...
def import_osm_way_data() -> None:
    ...
    for station_osm_record in station_osm_records:
        station = session.query(Station).filter(Station.code == station_osm_record.station_code).one_or_none()

        if not station:
            logger.error(f"Could not find station {station_osm_record.station_code}")
            continue

        if not station.location:
            logger.error(f"Station {station_osm_record.station_code} does not have a location")
            continue
        ...
        logger.info(f"Updated station: {station.code}")
...
def import_osm_ways() -> int:
    ...
    stations_with_ways = session.query(Station).join(Location).filter(Location.osm_way_id.isnot(None)).all()
    ...
    for station in stations_with_ways:
        ...
        try:
            ...
            logger.error(f"get osm wkt error: {e}")
            pass
        ...
        logger.info(f"Updated boundary geom from OSM for station: {station.code}")
