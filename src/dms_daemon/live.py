...
class SCADA_File(NemFile):
	...
	def __init__(self,fileobject = '/data/marble/nemweb/ARCHIVE/DispatchIS_Reports/PUBLIC_DISPATCHIS_20170208.zip'):
...
def get_zip(href="/Reports/CURRENT/Dispatch_SCADA/PUBLIC_DISPATCHSCADA_201708161600_0000000286435695.zip",log=True):
...
def web_log(response):
	...
	timestamp = datetime.datetime.strptime(webtimestamp, "%a, %d %b %Y %H:%M:%S GMT") + datetime.timedelta(0,10*3600)
...
def main():
	...
	pattern = re.compile("/Reports/CURRENT/Dispatch_SCADA/PUBLIC_DISPATCHSCADA_{0}_[0-9]{{16}}.zip".format(DI_timestring))
...
def dispatch_scada_proc(sf,df):
	for i,row in sf.DISPATCH_UNIT_SCADA.iterrows():
		try:
			...
			meta_pairs = ['{0}:{1}'.format(i,j) for i,j in zip(meta_cols.lower().split(","),meta)]
			statsd.gauge("unit.power", scadavalue,tags=meta_pairs)		
			statsd.gauge("unit.marketvalue", scadavalue*price/12.,tags=meta_pairs)		
		except:
			print("missing")
			pass
...
def dispatch_file_proc(df):
	...
	dc['VALUE']=dc.RRP*dc.TOTALDEMAND /12.
	for i,row in dc.iterrows():
		statsd.gauge("dispatch.totaldemand", row.TOTALDEMAND,tags=["regionid:{0}".format(row.REGIONID)])
		statsd.gauge("dispatch.netinterchange", row.NETINTERCHANGE,tags=["regionid:{0}".format(row.REGIONID)])
		statsd.gauge("dispatch.price", row.RRP,tags=["regionid:{0}".format(row.REGIONID)])
		statsd.gauge("dispatch.marketvalue", row.VALUE,tags=["regionid:{0}".format(row.REGIONID)])

def link_getter(page,DI_timestring):
	pattern = re.compile("/Reports/CURRENT/DispatchIS_Reports/PUBLIC_DISPATCHIS_{0}_[0-9]{{16}}.zip".format(DI_timestring))
	links = re.findall(pattern,page.text)
	return links
