...
class ZipFileStreamer(ZipFile):
	...
	def __init__(self,filename = '/data/marble/nemweb/ARCHIVE/DispatchIS_Reports/PUBLIC_DISPATCHIS_20170208.zip'):
...
def zip_streams(fileobject='/data/marble/nemweb/ARCHIVE/DispatchIS_Reports/PUBLIC_DISPATCHIS_20140208.zip'):
...
class NemFile():
	...
	def __table_reader(self,fileobject):
		...
		self.__dict__ = {table:pd.read_csv(BytesIO(self.__dict__[table])) for table in self.__dict__}	
