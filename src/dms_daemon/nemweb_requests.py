...
from dms_daemon import live, market_notice_reader, orm, CONFIG
from dms_daemon.nemweb_reader import NemFile,ZipFileStreamer

def slack_dms_webhook(message):
	json_text = "{0} \n ({1})".format(message,datetime.datetime.now().strftime("%Y-%m-%d %H:%M"))	
	r = requests.post('https://hooks.slack.com/services/{0}'.format(config['slack_hooks']['error_message'], json={"text":"{0}".format(json_text)})

def get_page(link = "http://www.nemweb.com.au/Reports/CURRENT/Dispatch_SCADA/"):
	return requests.get(link)

def get_zip(href="/Reports/CURRENT/Dispatch_SCADA/PUBLIC_DISPATCHSCADA_201708161600_0000000286435695.zip"):
		...
		if key in df.columns:
			...
			try:			
				...
				if (key == 'DUID') & (name == 'DISPATCH_UNIT_SCADA'):
					orm.engine_meta.execute("INSERT INTO DUID (DUID) VALUES ({0})".format(e))
				else:
					raise e			
...
def dispatch_scada():
	...
	pattern = re.compile("/Reports/CURRENT/Dispatch_SCADA/PUBLIC_DISPATCHSCADA_([0-9]{12})_[0-9]{16}.zip")
...
def rooftop_actual():
	...
	pattern = re.compile("/Reports/Current/ROOFTOP_PV/ACTUAL/PUBLIC_ROOFTOP_PV_ACTUAL_MEASUREMENT_([0-9]{14})_[0-9]{16}.zip")
	...
	for match in pattern.finditer(page.text):
		...
		if dt > d + datetime.timedelta(0,1800):
			#print (match.group(0))
			nemfile = process_zip(match.group(0))
			nemfile.ROOFTOP_ACTUAL = nemfile.ROOFTOP_ACTUAL[nemfile.ROOFTOP_ACTUAL.REGIONID.isin(['NSW1', 'QLD1', 'SA1', 'TAS1', 'VIC1'])]
...
def dispatch_unit_solution():
	...
	pattern = re.compile("/Reports/Current/Next_Day_Dispatch/PUBLIC_NEXT_DAY_DISPATCH_([0-9]{8})_[0-9]{16}.zip")
	
	for match in pattern.finditer(page.text):
		dt = datetime.datetime.strptime(match.group(1),"%Y%m%d")
		if dt.date() >= d.date():	#not that same as metered gen - should probably make them consistent
			nemfile = process_zip(match.group(0))
...
def dispatch_unit_duplicate(link="http://www.nemweb.com.au/Reports/Current/Next_Day_Dispatch/DUPLICATE/"):
	d = datetime.datetime(2020,4,1)
	page = get_page(link)
	tables = ['DISPATCH_UNIT_SOLUTION']

	pattern = re.compile("/Reports/Current/Next_Day_Dispatch/DUPLICATE/PUBLIC_NEXT_DAY_DISPATCH_([0-9]{8})_[0-9]{16}.zip")
...
def metered_data_gen_duid():
	...
	pattern = re.compile("/Reports/Current/Next_Day_Actual_Gen/PUBLIC_NEXT_DAY_ACTUAL_GEN_([0-9]{8})_[0-9]{16}.zip")	
...
def rooftop_forecast(d):
	...
	pattern = re.compile("/Reports/Current/ROOFTOP_PV/FORECAST/PUBLIC_ROOFTOP_PV_FORECAST_([0-9]{14})_[0-9]{16}.zip")
...
def dispatch_is():
	...
	pattern = re.compile("/Reports/CURRENT/DispatchIS_Reports/PUBLIC_DISPATCHIS_([0-9]{12})_[0-9]{16}.zip")
...
def trading_is():
	...
	pattern = re.compile("/Reports/CURRENT/TradingIS_Reports/PUBLIC_TRADINGIS_([0-9]{12})_[0-9]{16}.zip")
...
def bidoffer_energy():
	...
	page = get_page(link="http://www.nemweb.com.au/Reports/CURRENT/Next_Day_Offer_Energy/")
	tables = ['OFFER_BIDDAYOFFER','OFFER_BIDPEROFFER']

	pattern = re.compile("/Reports/CURRENT/Next_Day_Offer_Energy/PUBLIC_NEXT_DAY_OFFER_ENERGY_([0-9]{8})_[0-9]{16}.zip")
...
def bidoffer_fcas():
	...
	pattern = re.compile("/Reports/CURRENT/Next_Day_Offer_FCAS/PUBLIC_NEXT_DAY_OFFER_FCAS_([0-9]{8})_[0-9]{16}.zip")
...
def predispatch():
	...
	d = seqno_to_dt(d)
	page = get_page(link="http://www.nemweb.com.au/Reports/CURRENT/PredispatchIS_Reports/")
	tables = ['PREDISPATCH_REGION_SOLUTION', 'PREDISPATCH_REGION_PRICES']

	pattern = re.compile("/Reports/CURRENT/PredispatchIS_Reports/PUBLIC_PREDISPATCHIS_([0-9]{12})_[0-9]{14}.zip")
...
def market_notices():
	...
	pattern = re.compile("/Reports/Current/Market_Notice/NEMITWEB1_MKTNOTICE_([0-9]{8}).R([0-9]{5})")
	
	for match in pattern.finditer(page.text):
		dt = datetime.datetime.strptime(match.group(1),"%Y%m%d")
		notice_dir = "/data/marble/nemweb/MARKET_NOTICES/{0}/".format(datetime.date(dt.year, dt.month,dt.day))
		...
		if os.path.exists(filename):		
			...
			market_notice_reader.check_notice(filename)

if __name__ == "__main__":
	for func in [trading_is,dispatch_is,dispatch_scada, market_notices, rooftop_actual, rooftop_forecast_duplicate]:
		try:
			func()
		except Exception as E:
			print (func)
			slack_dms_webhook("`{0}` failed from file `{1}`\n{2}\n{3}".format(func.__name__, __file__, E, E.args))
