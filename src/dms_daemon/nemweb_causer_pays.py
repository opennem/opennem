...
def insert_fcas(csvs, table= 'fcas_201802'):
    ...
    for csv in csvs:
        df = pd.read_csv(csv, parse_dates=[0], index_col=[0], header=None, names = ["SETTLEMENTDATE","ELEMENT_ID","VARIABLE_ID","VALUE","QUALITY"])

        if df.index.dtype.name=="object":
            df.dropna(inplace=True)
            slack_dms_webhook("Issue with FCAS csv (wrong datetime)\n  (CSV file starting at {})".format(df.index[0]))
...
def causer_pays():
    ...
    page = nemweb_requests.get_page(link="http://www.nemweb.com.au/Reports/Current/Causer_Pays/")
...
def create_table(table_str="fcas_201801"):
    with orm.engine3.connect() as conn:
        ...
        conn.execute("ALTER TABLE `nemweb_causer_pays`.`{0}`  "\
...
def auto_table(error_msg):
    ...
    if len(match.groups()) ==1:
        ...
        raise Exception ("Auto_table error: wrong table name format", match.group(0).split("'")[1])

def find_new_id(df):
    for element in df.ELEMENT_ID.unique():
        cursor = orm.engine3.execute("SELECT * FROM elements where id = {0}".format(element)
...
def archive_file_reader(d = datetime.datetime(2017,2,10, 15,25)):
    filename = '/data/marble/nemweb/DATA_ARCHIVE/FCAS_Causer_Pays/{0.year}/FCAS_Causer_Pays_{1}/FCAS_{2}.zip'.format(d,d.strftime("%Y_%m"),d.strftime("%Y%m%d%H%M"))
    return filename
