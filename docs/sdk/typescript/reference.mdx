...
- `metrics`: Array of market metrics including:
  ...
  - Curtailment power (MW): `"curtailment"`, `"curtailment_solar"`, `"curtailment_wind"`
  - Curtailment energy (MWh): `"curtailment_energy"`, `"curtailment_solar_energy"`, `"curtailment_wind_energy"`
- `params`: Optional parameters (similar to getNetworkData)
...
const { datatable } = await client.getMarket(
  "NEM", 
  ["curtailment_solar", "curtailment_wind", "curtailment"],
  {
    interval: "5m",
    dateStart: "2024-01-01T00:00:00",
...
const { datatable } = await client.getMarket(
  "NEM",
  ["curtailment_solar_energy", "curtailment_wind_energy", "curtailment_energy"],
  {
    interval: "1d",
