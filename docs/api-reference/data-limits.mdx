...
In addition to the per-request interval ranges above, your plan determines how far back in time you can query.

| Plan | Historical window |
|------|-------------------|
| Community | 2 years |
| Academic | Full history |
| Enterprise | Full history |

Requests whose `date_start` falls outside your plan's window return a 400 with a detail message:

```json
{
  "detail": "Date start is too far in the past. Your plan allows data from the last 730 days."
}
```

If you need access to data older than 2 years, upgrade your plan or [contact us](https://platform.openelectricity.org.au).
...
// Get power generation data for the last 30 days
const end = new Date()
