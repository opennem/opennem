...
- **Pre-2009 demand market value** - Corrected NEM demand-weighted price before July 2009: the half-hourly trading price is now carried forward across the 5-minute buckets (`locf`), fixing demand market value that was understated by roughly 6x during the 30-minute price era ([#309](https://github.com/opennem/opennem/issues/309))

## Documentation

- **New docs platform** - Documentation has moved from Mintlify to [Tangly](https://docs.openelectricity.org.au), a self-hosted open-source docs framework that renders the existing docs project unmodified, with a direct branch-to-environment deploy pipeline
