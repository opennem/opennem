...
- **Max Generation Tracking** - Historical peak generation capacity tracking per facility unit ([#461](https://github.com/opennem/opennem/issues/461))
  ...
  - Backfill support for historical peak data
...
- **Unified HTTP Client** - New rnet-based HTTP client replacing legacy libraries ([#466](https://github.com/opennem/opennem/issues/466), [#467](https://github.com/opennem/opennem/issues/467), [#475](https://github.com/opennem/opennem/issues/475))
  ...
  - Replaced httpx, requests, and curl_cffi with unified rnet client
  - Features include custom retry logic, browser mimicking, proxy support, and random user agents
  - Migrated all external clients (APVI, BOM, AEMO notices, Slack)
  - Improved reliability and reduced dependency footprint
...
- **Granian ASGI Server** - Application server upgraded from Hypercorn to Granian ([#476](https://github.com/opennem/opennem/issues/476))
  - Enhanced performance with configurable worker threads
  - Improved development experience with watch mode
...
- **Facility API Enhancements** - Expanded facility endpoint capabilities ([#464](https://github.com/opennem/opennem/issues/464))
  ...
  - Versioning and caching enabled for better performance

- **CMS Sync Improvements** - Enhanced content management synchronization ([#453](https://github.com/opennem/opennem/issues/453))
  - Webhook sync by cms_id for reliable updates
  - Track created and updated timestamps from CMS
  - NPI ID and OSM way ID synchronization
  - Automated CMS update reports and scheduling
