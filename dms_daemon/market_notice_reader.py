...
def check_notice(fn, notice_types = ['POWER SYSTEM EVENTS','GENERAL NOTICE','RESERVE NOTICE','MARKET INTERVENTION']):
    ...
    if market_notice[12].find("LRC/LOR1/LOR2/LOR3") != -1:
        try:
            ...
            r = requests.post('https://hooks.slack.com/services/{0}'.format(config['slack_hooks']['error_message'], json={"text":E.args[0]})
            raise E
...
def slack_alert(market_notice):
    ...
    r = requests.post('https://hooks.slack.com/services/{0}'.format(config['slack_hooks']['market_notice'], json={"text":message})
...
def lor_notice(market_notice, lor_match):
    ...
    if lor_category_match == None:
        ...
        lor_cancellation_template(int(lor_match.group(1)), region_match.group(1), lor_category_match.group(1))
    else:
...
def lor_update(market_notice, lor_match, region_match, lor_category_match):
    ...
    try:
        ...
        try: 
            ...
            try:
                ...
                try:
                    from_time, to_time, from_time2, to_time2 = multi_time_parse(string="".join(market_notice))
                except:
    ...
    lor_template( int(lor_match.group(1)),\
...
def reserve_required(market_notice):
    reserve_requirement_pattern = re.compile("reserve requirement is (\d{3,4}|\d{1},\d{3}) MW")
    reserve_requirement_match = re.search(reserve_requirement_pattern, "".join(market_notice))
    if reserve_requirement_match == None:
        raise Exception("Bad reserve requirement format")
    return reserve_requirement_match.group(1).replace(",","")

def lor_template(level, category, region, lor_type, from_dt, to_dt, reserve_req, reserve_avail, from_dt2=None, to_dt2=None):
    colors = {1: "#ffc100",
              2: "#ff7400",
              3: "ff0000" }
    title = "LOR{0} {2} for {1} ({3})".format(level, region, category.lower(), lor_type.lower())
    details = "({0}MW shortfall between {1} and {2})".format(reserve_req - reserve_avail, from_dt, to_dt)
    if from_dt2 != None:
            details = "({0}MW shortfall between {1} to {2}, and {3} to {4})".format(reserve_req - reserve_avail, from_dt, to_dt, from_dt2, to_dt2)
    r = requests.post('https://hooks.slack.com/services/{0}'.format(config['slack_hooks']['reserve_notice'], json={"attachments":[{"title":title, "color":colors[level]},{"text":details}]})
...
def lor_cancellation_template(level, region, category):
    ...
    r = requests.post('https://hooks.slack.com/services/{0}'.format(config['slack_hooks']['reserve_notice'], json={"attachments":[{"title":title, "color":colors[level]}]})


def time_parse(string, start="(F|f)rom"):
    time_pattern = re.compile("{0} (\d{{2}})(\d{{2}}) hrs (\d{{2}})/(\d{{2}})/(\d{{4}})".format(start))
...
def time_parse2(string):
    time_pattern = re.compile("(F|f)rom (\d{2})(\d{2}) hrs (T|t)o (\d{2})(\d{2}) hrs (\d{2})/(\d{2})/(\d{4})")
    time_match = re.search(time_pattern, string)
    _, H1, M1, _, H2, M2, d, m, y = time_match.groups()
    return datetime.datetime(int(y),int(m),int(d),int(H1),int(M1)), datetime.datetime(int(y),int(m),int(d),int(H2),int(M2))

def time_parse3(string):
    time_pattern = re.compile("(F|f)rom (\d{2})(\d{2}) hrs (T|t)o (\d{2})(\d{2}) hrs on (\d{2})/(\d{2})/(\d{4})")
    time_match = re.search(time_pattern, string)
    _, H1, M1, _, H2, M2, d, m, y = time_match.groups()
    return datetime.datetime(int(y),int(m),int(d),int(H1),int(M1)), datetime.datetime(int(y),int(m),int(d),int(H2),int(M2))

def multi_time_parse(string):
    time_pattern = re.compile("(F|f)rom (\d{2})(\d{2}) hrs (T|t)o (\d{2})(\d{2}) hrs and (\d{2})(\d{2}) hrs (T|t)o (\d{2})(\d{2}) hrs (\d{2})/(\d{2})/(\d{4})")
    time_match = re.search(time_pattern, string)
    _, H1, M1, _, H2, M2, H3, M3, _, H4, M4, d, m, y = time_match.groups()
    return datetime.datetime(int(y),int(m),int(d),int(H1),int(M1)), datetime.datetime(int(y),int(m),int(d),int(H2),int(M2)), datetime.datetime(int(y),int(m),int(d),int(H3),int(M3)), datetime.datetime(int(y),int(m),int(d),int(H4),int(M4))
...
def send(market_notice):
    ...
    server.sendmail(CONFIG['gmail']['username'], CONFIG['gmail']['send_to'], msg.as_string())
    server.close()

test_file = '/data/marble/nemweb/MARKET_NOTICES/2017-10-24/59591.txt'

line_12 = [ 'Details of Non-conformance/Conformance',
