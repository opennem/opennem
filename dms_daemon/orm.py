...
engine = create_engine("mysql://{username}:{password}@{hostname}/nemweb?unix_socket={unix_socket}".format(**CONFIG["mysql"]))
engine2 = create_engine("mysql://{username}:{password}@{hostname}/nemweb_live?unix_socket={unix_socket}".format(**CONFIG["mysql"]))
engine3 = create_engine("mysql://{username}:{password}@{hostname}/nemweb_causer_pays?unix_socket={unix_socket}".format(**CONFIG["mysql"]))
engine_meta = create_engine("mysql://{username}:{password}@{hostname}/nemweb_meta?unix_socket={unix_socket}".format(**CONFIG["mysql"]))
engine_derived = create_engine("mysql://{username}:{password}@{hostname}/nemweb_derived?unix_socket={unix_socket}".format(**CONFIG["mysql"]))
...
keys = AttrDict({i:load_keys(i) for i in ['REGIONID', 'INTERCONNECTORID', 'BIDTYPE', 'RUNTYPE']})
keys['DUID'] = AttrDict(engine_meta.execute("SELECT {0},ID FROM {0}".format('DUID')).fetchall())
...
class Archive:
	def __init__(self, archive_name):
		...
		self.filelist = sorted(glob.glob("/data/marble/nemweb/ARCHIVE/{}/*".format(archive_name)))
	...
	def __load_table_data(self):
		filename,sample_file = self.unzip_archive(self.filelist[0]).next()
...
class ArchiveInserter(Archive):
	...
	def insert_dataframe(self,df,map_from="DUID",map_dict=duid_dict):
		...
		df[self.cols].to_sql(name='DISPATCH_UNIT_SOLUTION',index=None,con=engine,if_exists='append')
...
class ArchiveUpdater(Archive):
	...
	def update_archive_file(self, archive_file,map_from="REGIONID", map_dict = region_dict):
		session = Session()
		...
		for zf_name, zipfile in self.unzip_archive(archive_file):
			...
			try:
				nemfile = self.unzip_zipfile(zipfile)
			except nemweb_reader.BadZipfile as E:
	...
	def legacy_filelist(self):
			...
			for year in range(2006,2012):
				self.filelist+=(glob.glob("/data/marble/nemweb/LEGACY/public_dispatchis/public_dispatchis_{0}/*.zip".format(year)))
...
class trading_price_updater(ArchiveUpdater):
		def __init__(self, archive_name="TradingIS_Reports"):
			...
			self.update_columns = [	u'ROP', u'RAISE6SECRRP', u'RAISE6SECROP',u'RAISE60SECRRP', 
							u'RAISE60SECROP', u'RAISE5MINRRP', u'RAISE5MINROP',
							u'RAISEREGRRP', u'RAISEREGROP', u'LOWER6SECRRP', u'LOWER6SECROP',
							u'LOWER60SECRRP', u'LOWER60SECROP', u'LOWER5MINRRP', u'LOWER5MINROP',
							u'LOWERREGRRP', u'LOWERREGROP']
...
class trading_regionsum_updater(ArchiveUpdater):
		def __init__(self, archive_name="TradingIS_Reports"):
			...
			self.index_columns = [	'SETTLEMENTDATE', 'REGIONID']
			self.update_columns = [	'LOWERREGLOCALDISPATCH', 'LOWER5MINLOCALDISPATCH','LOWER60SECLOCALDISPATCH','LOWER6SECLOCALDISPATCH',
									'RAISE5MINLOCALDISPATCH', 'RAISEREGLOCALDISPATCH', 'RAISE60SECLOCALDISPATCH', 'RAISE6SECLOCALDISPATCH']
...
class dispatch_price_updater(ArchiveUpdater):
		def __init__(self, archive_name="DispatchIS_Reports"):
			...
			self.update_columns = [ u'ROP', u'APCFLAG',u'RAISE6SECRRP', u'RAISE6SECROP', u'RAISE6SECAPCFLAG', u'RAISE60SECRRP',
								   u'RAISE60SECROP', u'RAISE60SECAPCFLAG', u'RAISE5MINRRP',
								   u'RAISE5MINROP', u'RAISE5MINAPCFLAG', u'RAISEREGRRP', u'RAISEREGROP',
								   u'RAISEREGAPCFLAG', u'LOWER6SECRRP', u'LOWER6SECROP',
								   u'LOWER6SECAPCFLAG', u'LOWER60SECRRP', u'LOWER60SECROP',
								   u'LOWER60SECAPCFLAG', u'LOWER5MINRRP', u'LOWER5MINROP',
								   u'LOWER5MINAPCFLAG', u'LOWERREGRRP', u'LOWERREGROP',
								   u'LOWERREGAPCFLAG']
...
class dispatch_regionsum_updater(ArchiveUpdater):
		def __init__(self, archive_name="DispatchIS_Reports"):
			...
			self.update_columns = [	'LOWERREGLOCALDISPATCH', 'LOWER5MINLOCALDISPATCH','LOWER60SECLOCALDISPATCH','LOWER6SECLOCALDISPATCH',
									'RAISE5MINLOCALDISPATCH', 'RAISEREGLOCALDISPATCH', 'RAISE60SECLOCALDISPATCH', 'RAISE6SECLOCALDISPATCH',
									'LOWERREGACTUALAVAILABILITY', 'LOWER5MINACTUALAVAILABILITY','LOWER60SECACTUALAVAILABILITY',
									'LOWER6SECACTUALAVAILABILITY', 'RAISE5MINACTUALAVAILABILITY', 'RAISEREGACTUALAVAILABILITY', 
									'RAISE60SECACTUALAVAILABILITY', 'RAISE6SECACTUALAVAILABILITY', 'AGGREGATEDISPATCHERROR']
...
class predispatch_price_updater(ArchiveUpdater):
		def __init__(self, archive_name="PreDispatchIS_Reports"):
			...
			self.update_columns = [ u'RAISE6SECRRP', u'RAISE60SECRRP', u'RAISE5MINRRP',u'RAISEREGRRP',u'LOWER6SECRRP',u'LOWER60SECRRP', u'LOWER5MINRRP', u'LOWERREGRRP']
			self.skip_files += ["PUBLIC_PREDISPATCHIS_201610121900_20161012183215.zip", "PUBLIC_PREDISPATCHIS_201610130000_20161012233150.zip"]
...
class bidday_updater(ArchiveUpdater):
		def __init__(self, archive_name="Next_Day_Offer_Energy"):
			...
			self.index_columns = [	'SETTLEMENTDATE', 'DUID', 'OFFERDATE','VERSIONNO','ENTRYTYPE','LASTCHANGED'] 
...
class ScadaInserter(Archive):
	def __init__(self, live_name):
		self.name = live_name
		self.filelist = sorted(glob.glob("/data/marble/nemweb/LIVE/{}/*".format(live_name)))
...
class RegionPriceInserter(Archive):
	...
	def key_map(self,map_from="REGIONID", map_dict = region_dict):
		for table in ['DISPATCH_PRICE']:
			df = self.nemfile.__dict__[table]
			df[map_from] = df[map_from].apply(lambda x: map_dict[x])
			df=df[[	'SETTLEMENTDATE','REGIONID','DISPATCHINTERVAL','RRP','ROP','APCFLAG','CUMUL_PRE_AP_ENERGY_PRICE','INTERVENTION','RAISE6SECRRP','RAISE6SECROP','RAISE6SECAPCFLAG','RAISE60SECRRP',
					'RAISE60SECROP','RAISE60SECAPCFLAG','RAISE5MINRRP','RAISE5MINROP','RAISE5MINAPCFLAG','RAISEREGRRP','RAISEREGROP','RAISEREGAPCFLAG','LOWER6SECRRP','LOWER6SECROP','LOWER6SECAPCFLAG',
					'LOWER60SECRRP','LOWER60SECROP','LOWER60SECAPCFLAG', 'LOWER5MINRRP','LOWER5MINROP','LOWER5MINAPCFLAG','LOWERREGRRP','LOWERREGROP','LOWERREGAPCFLAG']]
...
class RegionSum(Archive):
	...
	def key_map(self,map_from="REGIONID", map_dict = region_dict):
		for table in ['DISPATCH_REGIONSUM']:
			df = self.nemfile.__dict__[table]
			df[map_from] = df[map_from].apply(lambda x: map_dict[x])
			df=df[[	'SETTLEMENTDATE','REGIONID','TOTALDEMAND','AVAILABLEGENERATION','AVAILABLELOAD','DEMANDFORECAST','DISPATCHABLEGENERATION','DISPATCHABLELOAD','NETINTERCHANGE','EXCESSGENERATION',
					'INITIALSUPPLY','CLEAREDSUPPLY','TOTALINTERMITTENTGENERATION','DEMAND_AND_NONSCHEDGEN','UIGF','SEMISCHEDULE_CLEAREDMW','DISPATCHINTERVAL','SEMISCHEDULE_COMPLIANCEMW','INTERVENTION',
					'LOWERREGLOCALDISPATCH','LOWER5MINLOCALDISPATCH','LOWER60SECLOCALDISPATCH','LOWER6SECLOCALDISPATCH','RAISE5MINLOCALDISPATCH','RAISEREGLOCALDISPATCH','RAISE60SECLOCALDISPATCH',
					'RAISE6SECLOCALDISPATCH','AGGREGATEDISPATCHERROR','RAISE6SECACTUALAVAILABILITY','RAISE5MINACTUALAVAILABILITY','RAISE60SECACTUALAVAILABILITY','RAISEREGACTUALAVAILABILITY',
					'LOWER6SECACTUALAVAILABILITY', 'LOWER5MINACTUALAVAILABILITY',	'LOWER60SECACTUALAVAILABILITY',	'LOWERREGACTUALAVAILABILITY']]
...
class TradingRegionSum(Archive):
	...
	def key_map(self,map_from="REGIONID", map_dict = region_dict):
		for table in ['TRADING_REGIONSUM']:
			...
			df[map_from] = df[map_from].apply(lambda x: map_dict[x])
			df=df[[	'SETTLEMENTDATE','REGIONID','TOTALDEMAND','AVAILABLEGENERATION','AVAILABLELOAD','DEMANDFORECAST','DISPATCHABLEGENERATION','DISPATCHABLELOAD','NETINTERCHANGE','EXCESSGENERATION',
					'INITIALSUPPLY','CLEAREDSUPPLY','TOTALINTERMITTENTGENERATION','DEMAND_AND_NONSCHEDGEN','UIGF','LOWERREGLOCALDISPATCH','LOWER5MINLOCALDISPATCH','LOWER60SECLOCALDISPATCH',
					'LOWER6SECLOCALDISPATCH','RAISE5MINLOCALDISPATCH','RAISEREGLOCALDISPATCH','RAISE60SECLOCALDISPATCH','RAISE6SECLOCALDISPATCH']]
...
class TradingPrice(Archive):
	...
	def key_map(self,map_from="REGIONID", map_dict = region_dict):
		for table in ['TRADING_PRICE']:
			df = self.nemfile.__dict__[table]
			df[map_from] = df[map_from].apply(lambda x: map_dict[x])
			df=df[[	'SETTLEMENTDATE','REGIONID','RRP','ROP','RAISE6SECRRP','RAISE6SECROP','RAISE60SECRRP','RAISE60SECROP','RAISE5MINRRP','RAISE5MINROP','RAISEREGRRP','RAISEREGROP','LOWER6SECRRP',
					'LOWER6SECROP','LOWER60SECRRP','LOWER60SECROP','LOWER5MINRRP','LOWER5MINROP','LOWERREGRRP','LOWERREGROP']]
...
class DispatchUnitSolution(Archive):
	def __init__(self, name = 'Next_Day_Dispatch'):
		self.name = name
		self.filelist = sorted(glob.glob("/data/marble/nemweb/ARCHIVE/{0}/*".format(name)))
...
class PreDispatchSensitivities(ArchiveInserter):
	def __init__(self, name="Predispatch_Sensitivities"):		
		...
		self.cols = Base.classes['PREDISPATCH_PRICESENSITIVITIES'].__table__.columns.keys()
		self.cols.remove('ID')
	...
	def insert_dataframe(self,df,map_from="REGIONID",map_dict=region_dict):
		...
		df[self.cols].to_sql(name='PREDISPATCH_PRICESENSITIVITIES',index=None,con=engine,if_exists='append')
	...
	def insert_archive_file(self,archive_file):
		for zf_name, zipfile in self.unzip_archive(archive_file):
			try:
				...
				print (zf_name)
				if zf_name in [	"PUBLIC_PREDISPATCH_SENSITIVITIES_20161227093218_0000000279137718.zip", 
